...
class ProteinLigandInteraction(BaseModel):
    interaction_type: InteractionType = Field(
        ..., description="Type of interaction between protein and ligand."
    )
    protein_residue_number: int = Field(
        ..., description="Residue number of the protein involved in the interaction."
    )
    protein_residue_type: ProteinResidueType = Field(
        ...,
        description="Amino acid identify of the residue involved in the interaction.",
    )
    to_sidechain: bool = Field(
        ...,
        description="Indicates if the interaction is to the sidechain of the protein residue (if False, it is to the backbone).",
    )
    count: int = Field(
        1, description="Count of the number of interactions of this type."
    )
    ligand_atom_type: AtomType = Field(
        "none", description="Type of the ligand atom involved in the interaction."
    )
    protein_atom_type: AtomType = Field(
        "none", description="Type of the protein atom involved in the interaction."
    )
    ligand_charge: FormalCharge = Field(
        "neutral",
        description="Formal charge of the ligand involved in the interaction.",
    )
    protein_charge: FormalCharge = Field(
        "neutral",
        description="Formal charge of the protein involved in the interaction.",
    )
    protein_atom_number: int = Field(
        -1,
        description="Atom number of the protein involved in the interaction. Optional, can be -1 if not applicable.",
    )
    ligand_atom_number: int = Field(
        -1,
        description="Atom number of the ligand involved in the interaction. Optional, can be -1 if not applicable.",
    )

    def to_json_dict(self) -> dict:
        return {
            k: v.value if k in [self.interaction_type, self.protein_residue_type] else v
...
def plip_constructor(plip: PLInteraction) -> list[ProteinLigandInteraction]:
    ...
    for interaction in plip.all_itypes:
        ...
        if interaction_type == "hbond":
            output_interaction_type = (
                InteractionType.HydrogenBondAcceptor
                if interaction.protisdon
                else InteractionType.HydrogenBondDonor
            )
            protein_atom_type = AtomType(
                interaction.dtype[0] if interaction.protisdon else interaction.atype[0]
            )
            ligand_atom_type = AtomType(
                interaction.atype[0] if interaction.protisdon else interaction.dtype[0]
            )
            protein_atom_number = (
                interaction.a_orig_idx
                if interaction.protisdon
                else interaction.d_orig_idx
            )
            ligand_atom_number = (
                interaction.d_orig_idx
                if interaction.protisdon
                else interaction.a_orig_idx
            )
            ...
            ligand_charge = (
                FormalCharge.Negative
                if interaction.protispos
                else FormalCharge.Positive
            )
            protein_charge = (
                FormalCharge.Positive
                if interaction.protispos
                else FormalCharge.Negative
            )
            ...
            raise NotImplementedError(
                f"Interaction type not implemented: {interaction_type} for interaction: {interaction}"
            )
        ...
        interactions.append(
            ProteinLigandInteraction(
                interaction_type=output_interaction_type,
...
class PLIntReport(BaseModel):
    ...
    report_id: str = Field(
        ...,
        description="Identifier for the structure, typically the input structure filename without extension.",
    )
    structure: str = Field(..., description="Path to the PDB structure file.")
    interactions: list[ProteinLigandInteraction] = Field(
        ...,
        description="List of Protein-Ligand Interaction objects found in the structure.",
    )

    @classmethod
    def from_complex_path(
        cls, complex_path: str | Path, ligand_id="UNK"
    ) -> "PLIntReport":
        """
    ...
    def to_csv(self, path: str | Path):
        ...
        df = pd.DataFrame.from_records(
            [json.loads(interaction.json()) for interaction in self.interactions]
        )
    ...
    def from_csv(cls, plint_report: str | Path) -> "PLIntReport":
        ...
        report_id = (
            ...
            else Path(plint_report).stem
        )
        pdb_file = (
            df["structure"].iloc[0] if "structure" in df.columns else str(plint_report)
        )

        interactions = [ProteinLigandInteraction(**row) for _, row in df.iterrows()]
        return PLIntReport(
            report_id=report_id, structure=pdb_file, interactions=interactions
        )
...
class FingerprintLevel(Enum):
    ...
    ByInteractionTypeAndResidueTypeAndBBorSC = (
        "ByInteractionTypeAndResidueTypeAndBBorSC"
    )
    ByInteractionTypeAndResidueTypeAndNumber = (
        "ByInteractionTypeAndResidueTypeAndNumber"
    )
...
class SimilarityScore(BaseModel):
    ...
    provenance: str = Field(
        ..., description="Method used to calculate the similarity score."
    )
    score: float = Field(
        ...,
        description="Similarity score between the two fingerprints, ranging from 0 to 1.",
    )
    number_of_interactions_in_reference: int = Field(
        ..., description="Total number of interactions in the reference fingerprint."
    )
    number_of_interactions_in_query: int = Field(
        ..., description="Total number of interactions in the query fingerprint."
    )
    number_of_interactions_in_intersection: int = Field(
        ..., description="Number of interactions common to both fingerprints."
    )
    number_of_interactions_in_union: int = Field(
        ..., description="Total number of unique interactions across both fingerprints."
    )
    ...
    def check_interaction_counts(cls, values):
        ...
        if not (num_intersection <= num_ref and num_intersection <= num_query):
            raise ValueError(
                f"Number of interactions in intersection ({num_intersection}) cannot exceed those in reference ({num_ref}) or query ({num_query})."
            )

        if not (num_union >= max(num_ref, num_query)):
            raise ValueError(
                f"Number of interactions in union ({num_union}) must be at least as large as the larger of the two fingerprints (ref {num_ref}, query {num_query})."
            )

        if not num_ref + num_query - num_intersection == num_union:
            raise ValueError(
                f"Union {num_union} must equal the sum of both fingerprints (ref {num_ref}, query {num_query}) minus the intersection ({num_intersection})."
            )

        return values


def calculate_tversky(
    fingerprint1: dict, fingerprint2: dict, alpha: float = 1, beta: float = 0
) -> SimilarityScore:
    """
    ...
    matched = sum(
        [min(fingerprint1.get(a, 0), fingerprint2.get(a, 0)) for a in fp_types]
    )

    # Calculate the number of interactions in the union
    union = sum(fingerprint1.values()) + sum(fingerprint2.values()) - matched
    union_v2 = sum(
        [max(fingerprint1.get(a, 0), fingerprint2.get(a, 0)) for a in fp_types]
    )
    if not union == union_v2:
        raise ValueError(
            f"Union calculation mismatch: {union} != {union_v2}. Please check the input fingerprints."
        )

    # Calculate the Tversky Index
    score = matched / (
        matched
        + alpha * (sum(fingerprint1.values()) - matched)
        + beta * (sum(fingerprint2.values()) - matched)
    )
    return SimilarityScore(
        provenance=f"Tversky_alpha{alpha:.2f}_beta{beta:.2f}",
        score=score,
        number_of_interactions_in_reference=sum(fingerprint1.values()),
        number_of_interactions_in_query=sum(fingerprint2.values()),
        number_of_interactions_in_intersection=matched,
        number_of_interactions_in_union=union,
    )
...
class InteractionScore(BaseModel):
    ...
    provenance: str = Field(
        ...,
        description="Level of detail used for the fingerprint comparison (e.g., ByInteractionType).",
    )
    number_of_interactions_in_query: int = Field(
        ..., description="Total number of interactions in the query structure."
    )
    number_of_interactions_in_reference: int = Field(
        ..., description="Total number of interactions in the reference structure."
    )
    number_of_interactions_in_intersection: int = Field(
        ..., description="Number of interactions common to both structures."
    )
    number_of_interactions_in_union: int = Field(
        ..., description="Total number of unique interactions across both structures."
    )
    tanimoto_coefficient: float = Field(
        ...,
        description="Tanimoto coefficient (α=β=1) measuring similarity between the structures, ranges from 0 to 1.",
    )
    tversky_index: float = Field(
        ...,
        description="Tversky index (α=1, β=0) measuring how well the query matches the reference, ranges from 0 to 1.",
    )
    reference_fingerprint: dict = Field(
        ...,
        description="Dictionary containing the interaction fingerprint of the reference structure.",
    )
    query_fingerprint: dict = Field(
        ...,
        description="Dictionary containing the interaction fingerprint of the query structure.",
    )

    @classmethod
    def from_fingerprints(
        cls, reference: PLIntReport, query: PLIntReport, level: FingerprintLevel
    ) -> "InteractionScore":
        ...
        return cls(
            provenance=level.value,
            number_of_interactions_in_query=tanimoto.number_of_interactions_in_query,
